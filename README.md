...
8. **Nova regra necessária para o botão "Terminar torneio"**: essa funcionalidade escreve o campo `stats` (torneios/vitorias/derrotas) no documento `users/{uid}` de CADA jogador registado que participou, não só no do próprio organizador — por isso a regra normal de "só o próprio uid pode escrever no seu users/{uid}" bloqueia isto. É preciso acrescentar ao `match /users/{uid}` do teu `firestore.rules` uma exceção que permita a atualização só do campo `stats`, só quando quem escreve é o organizador/moderador do torneio a que esse jogador pertence e o torneio está a passar a `status == "terminado"`. Nas regras deste projeto isso corresponde a algo como (ajusta `isAdmin()`/nomes de função ao que já tens):
   ```
   allow update: if request.auth != null
     && request.resource.data.diff(resource.data).affectedKeys().hasOnly(['stats'])
     && exists(/databases/$(database)/documents/players/$(uid + '_algumTorneio')); // ver nota abaixo
   ```
   Como a app não sabe à partida em que torneio(s) o uid jogou a partir da regra de `users/{uid}` isolada, a forma mais simples e segura é permitir que **qualquer utilizador autenticado escreva SÓ no campo `stats`** de `users/{uid}` (nunca nos outros campos), e confiar que só o botão "Terminar torneio" (visível apenas ao organizador/moderador) chama esta escrita — o mesmo padrão de confiança já usado noutras regras deste projeto (ex: `participantUids`). Sem publicar esta regra, o botão continua a terminar o torneio na mesma, mas avisa que não conseguiu guardar as estatísticas de alguns jogadores.
