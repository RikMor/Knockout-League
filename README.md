...
8. **Nova regra para o botão "Terminar torneio"**: já está incluída no `firestore.rules` deste zip (bloco `match /users/{uid}`) — permite a qualquer utilizador autenticado atualizar só o campo `stats` de `users/{uid}` de outros jogadores, o que essa funcionalidade precisa para gravar torneios/vitórias/derrotas no perfil de cada participante. Publica-a com `firebase deploy --only firestore:rules` (ponto 7 acima) para o botão conseguir gravar as estatísticas.
